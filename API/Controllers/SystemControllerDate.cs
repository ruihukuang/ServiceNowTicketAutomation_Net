...
namespace API.Controllers
{
    ...
    public class SystemDateController(AppDbContext context,IHttpClientFactory httpClientFactory) : BaseApiController
    {

        [HttpGet]
        public IActionResult Get()
        {
            return Ok("AI system processing API with date filters is working!");
        }


        [HttpPost("AI_system_date")]
        public async Task<IActionResult> SendDataToApi([FromQuery] string? year = null, [FromQuery] string? month = null)
        {
            ...
            Console.WriteLine("Checking for activities that need AI system identification...");
            
            // CHANGED: Start with base query and apply filters
            var query = context.Activities.AsQueryable();

            // CHANGED: Apply year filter if provided
            ...
            // CHANGED: Apply month filter if provided
            ...
            // CHANGED: Ensure both year and month are specified in the data (not null or empty)
            query = query.Where(a => 
                !string.IsNullOrEmpty(a.OpenDate_Year) && 
                !string.IsNullOrEmpty(a.OpenDate_Month));

            // CHANGED: REMOVED the System_AI check - now processing ALL activities with LongDescription
            // regardless of whether they already have System_AI populated
            var activitiesNeedingProcessing = await query
                .Where(a => !string.IsNullOrWhiteSpace(a.LongDescription)) // ONLY check for LongDescription existence
                .Select(a => new { a.Id, a.LongDescription, a.System_AI, a.OpenDate_Year, a.OpenDate_Month })
                .ToListAsync();

            Console.WriteLine($"=== PROCESSING CHECK ===");
            Console.WriteLine($"Activities needing AI system identification: {activitiesNeedingProcessing.Count}");
            
            // CHANGED: Show filter details
            ...
            {
                Console.WriteLine($"ID: {activity.Id}, Date: {activity.OpenDate_Year}-{activity.OpenDate_Month}, Current System_AI: '{activity.System_AI}'");
            }
            Console.WriteLine("======================================");
            ...
            {
                var message = "No activities need AI system identification";
                if (!string.IsNullOrEmpty(year) || !string.IsNullOrEmpty(month))
                {
                    message += $" for the specified filters (Year: {year ?? "Any"}, Month: {month ?? "Any"})";
                }
                message += " - no activities with LongDescription found";
            ...
            Console.WriteLine($"Processing {activitiesNeedingProcessing.Count} activities that need system identification");
            // CHANGED: Show filter criteria in processing message
            if (!string.IsNullOrEmpty(year) || !string.IsNullOrEmpty(month))
            {
                Console.WriteLine($"Filter criteria - Year: {year ?? "Any"}, Month: {month ?? "Any"}");
            }

            // Process only activities that need system identification
            foreach (var activity in activitiesNeedingProcessing)
            {
            ...
            return Ok(new { 
                Message = "AI system processing for selected period completed", 
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    try
                    {
                        ...
                        Console.WriteLine($"Generated system for activity {Id} (length: {finalSummary.Length})");
